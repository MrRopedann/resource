...
function SendExchangeTData( character, npc, exchangeData )
	local packet = GetPacket()
...
local statelv = 0
local statetime = 0 --Çàäåðæêà (â ñåêóíäàõ)
         SafeSale( character, index, count )
AddState ( character, character, 5, statelv, statetime )
return LUA_TRUE
else
SystemNotice(character, "Ïðîäàâàòü ìîæíî êàæäóþ 1 ñåêóíäó! ")
return LUA_FALSE
end
...
SystemNotice(character, "Ïîêóïàòü ìîæíî ðàç â 1 ñåêóíäó! ")
return LUA_FALSE
end
end
...
function MsgProc( character, npc, rpk, page, trade, missionlist, exchangeData )
	...
	elseif usCmd == CMD_CM_FUNCITEM then --¹¦ÄÜÒ³Ñ¡ÏîÃüÁîÐÅÏ¢´¦Àí
		...
		elseif item.func == VIPSystem.Display then
			return VIPSystem.Display(character)	
