...
--	return
	...
	local Item_CanGet = GetChaFreeBagGridNum ( role )
	if Item_CanGet < 3 then
		SystemNotice(role ,"Для открытия необходимо 2 свободных слота ")
		UseItemFailed ( role )
		return
	end
	if lv < 5 then
		SystemNotice(role ,"Для открытия требуется 5 уровень ")
		UseItemFailed ( role )	
	else
...
function ItemUse_XRBOX1( role, Item )
	...
	if Item_CanGet < 9 then
		SystemNotice(role ,"Для открытия необходимо 8 свободных слотов ")
		UseItemFailed ( role )
		return
	end
	if lv < 10 then
		SystemNotice(role ,"Требуется 10 уровень ")
		UseItemFailed ( role )	
	elseif job == 1 then  
...
function ItemUse_XRBOX2( role, Item )
	...
	if Item_CanGet < 6 then
		PopupNotice(role ,"Для открытия необходимо 6 свободных слотов. ")
		UseItemFailed ( role )
		return
	end
	if lv < 15 then
		PopupNotice(role ,"Чтобы открыть сундук, вы должны иметь 15+ уровень. ")
		UseItemFailed ( role )	
	elseif job == 1 then  
...
function ItemUse_XRBOX3( role, Item )
	...
	if lv < 20 then
		SystemNotice(role ,"Требуется 20 уровень ")
		UseItemFailed ( role )	
	elseif job == 1 then  
...
local job = GetChaAttr(role, ATTR_JOB)
	...
	if lv < 25 then
		SystemNotice(role ,"Требуется 25 уровень ")
		UseItemFailed ( role )	
	elseif job == 1 then  
...
function ItemUse_XRBOX5( role, Item )
	...
	if lv < 30 then
		SystemNotice(role ,"Требуется 30 уровень ")
		UseItemFailed ( role )	
	elseif job == 1 then  
...
function ItemUse_XRBOX6( role, Item )
	...
	if lv < 35 then
		SystemNotice(role ,"Требуется 35 уровень ")
		UseItemFailed ( role )	
	elseif job == 1 then  
...
function ItemUse_XRBOX7( role, Item )
	...
	if lv < 40 then
		SystemNotice(role ,"Требуется 40 уровень ")
		UseItemFailed ( role )	
	elseif job == 9 then  
...
function ItemUse_XRBOX11( role, Item )
	...
	if Item_CanGet < 5 then
		SystemNotice(role ,"Для открытия необходимо 4 свободных слота ")
		UseItemFailed ( role )
		return
	end
	if lv < 80 then
		SystemNotice(role ,"Требуется 80 уровень ")
		UseItemFailed ( role )	
	elseif job == 8 then  
...
--Большой Усилитель стремлений (ID 3095)
...
function ItemUse_WeiXing( role , Item )
	...
	if ChaStateLv > 0 then
		PopupNotice ( role , "Эффект Ягоды Новичка уже наложен! " )
		UseItemFailed ( role )
		return
	end
	local lv_star = GetChaAttr( role , ATTR_LV) 
	if lv_star > 150 then
		PopupNotice( role , "Можно использовать только игрокам ниже 150 уровня. " )
		UseItemFailed ( role )
		return
	end
	local statetime = 360
    ...
    PlayEffect (role, 943 )      -- Заставляем проигрывать эффект при открытии сундука
end
...
SystemNotice( role ,"При попытке улучшения доспех был сильно поврежден поврежден, характеристики значительно снижены! ")
SetItemAttr( body, ITEMATTR_VAL_DEF, 1)
RefreshCha ( role )
end
end
...
function Item_Use_Pendowo(role,Item)
    ...
    if Cha_Boat ~=  nil then
        ...
        SystemNotice( role ,"Можно улучшать только когда вещь в 1 слоте! ")
        UseItemFailed ( role )
        return
        end
    ...
    if Item_ID == 7477 then --Attack (Puntodewo)
            ...
            local Limit    = 32000    --Limit value you want (Max 32767)
			 local effect = 587              -- ID эффекта из sceneffectinfo.txt
			 PlayEffect (role, effect )      -- Заставляем проигрывать э
            if (ItemAttrI >= Limit or ItemAttrII >= Limit) then
                    SystemNotice(role,"Достигнут лимит улучшения!")
                UseItemFailed ( role )
                return
            end
            ...
            if (FinalAttrI >= Limit or FinalAttrII >= Limit) then
                ...
                SystemNotice(role,"Успешное добавление +"..Bonus.."	атаки для вашего оружия ")
                RefreshCha(role)
			else
                SystemNotice(role,"Только оружие можно улучшать!")
            UseItemFailed ( role )
            return
			end
			end
...
function Item_Use_Pendowoq(role,Item)
	...
	if Item_ID == 7478 then --Defense (Werkudoro)
            ...
            if (FinalAttr >= Limit) then
			  ...
			  SystemNotice(role,"Успешное добавление +"..Bonus.." защиты для вашей экипировки ")
              RefreshCha(role)
			else
            SystemNotice(role,"Только доспех,ботинки,перчатки и шапку можно улучшать!")
            UseItemFailed ( role )
            return
            end
			end
...
function Item_Use_Pendowow(role,Item)
    ...
    if Item_ID == 7479 then --Phy Resist (Arjuno)
            ...
            if (FinalAttr >= Limit) then
             ...
             SystemNotice(role,"Успешное добавление +"..Bonus.." сопротивления для вашей экипировки ")
             RefreshCha(role)
			 else
			SystemNotice(role,"Только доспех и тату можно улучшать!")
            UseItemFailed ( role )
            return
            end
			end
...
function Item_Use_Pendowoe(role,Item)
    ...
    if Item_ID == 7480 then --Dodge (Nakulo)
            ...
            if (FinalAttr >= Limit) then
			 ...
			 SystemNotice(role,"Успешное добавление +"..Bonus.." уклонения для ваших ботинок ")
             RefreshCha(role)
			 else
            SystemNotice(role," Только ботинки можно улучшать! ")
            UseItemFailed ( role )
            return
            end
			end
...
function Item_Use_Pendowor(role,Item)
    ...
    if Item_ID == 7481 then --Hit rate (Sadewo)
            ...
            if (FinalAttr >= Limit) then
                ...
                SystemNotice(role,"Успешное добавление +"..Bonus.." попадания для ваших перчаток ")
                RefreshCha(role)
            else

                SystemNotice(role,"Только перчатки можно улучшать!")
            UseItemFailed ( role )
            return
            end
    end
...
SystemNotice(role , "Этот фрукт нельзя использовать, если Ваша фея меньше 500 уровня,а так же если выше 800 уровня " )
UseItemFailed ( role )
return
end
if Item_type == 58 and Item_Traget_Type == 59 then
...
SystemNotice(role , "Этот фрукт нельзя использовать, если Ваша фея меньше 800 уровня,а так же если выше 1000 уровня " )
UseItemFailed ( role )
return
end
if Item_type == 58 and Item_Traget_Type == 59 then
...
-- Фрукт на стр +2 (от 1000 до 1200)
...
-- Фрукт на спр +2 (от 1000 до 1200)
...
function ItemUse_mewsfruitt (role, Item)
	...
	elseif sc <=100 then
		GiveItem ( role , 0 ,7527 , 1 , 20 )                    ---------------ХжєЪБъЦ®Зы   »сµГ»ъВК14%
...
--7658	Камень Телепортации Шайтана
...
function ItemUse_checkorbq ( role, Item )
	if GetChaFreeBagGridNum (role) < 1 then
		HelpInfo(role, 0, "Чтобы открыть ["..GetItemName(GetItemID(Item)).."] нужно иметь 1 свободных слот в инвентаре! ")
		UseItemFailed ( role )
		return
	end
...
function ItemUse_Kontrakt (role, Item )
	...
	if (mob_id > 0) then
		if (kill_count > 0) then
			HelpInfo(role, 0, "Для выполнения контракта осталось убить x "..kill_count.." монстров.")
			UseItemFailed(role)
		else
			...
			for CheckALLKontr in KonPrize do
				...
				if (kontrakt_num == KonPrize[CheckALLKontr][1]) then
					...
					if mob_id == KonPrize[CheckALLKontr][2] then
						...
						PopupNotice(role, "Выдача награды по контракту ["..kontrakt_num.."] - "..PrizeName.."!")
					end
				end
			end
			--PopupNotice(role, "Выдача награды по контракту ["..kontrakt_num.."] - "..GetItemName(Kontr_Prize).."!")
		end
	else
		PopupNotice(role, "Контракт ещё не активен...")
		UseItemFailed(role)
	end
end
...
--Усиления скорости бега новичка
...
--Сундук Пещерного обитателя (ID 5200)
function ItemUse_PeshObit ( role , Item )
	local Random = math.random(1,100)
	local data
	Items = {}
	-- Сундук Рун Кэль(10шт)
	Items[1] = {7450} --25%
	-- Расписка на Реф
	Items[2] = {3877} --25%
	-- Черная жемчужина
	...
	elseif (Random >= 76 and Random <= 96 ) then
		SystemNotice( role, "Вы ничего не получили! ")
	end
	--Купюра на 50к - 100%
	GiveItem(role, 0, 7491, 1, 4)
end 

--Сундук Пещерного обитателя (ID 5201)
function ItemUse_PeshShit ( role , Item )
	local Random = math.random(1,100)
	local data
	Items = {}
	-- Сундук Рун Кэль(50 шт)
	Items[1] = {7692} --33%
	-- Ключ Пурпурного Кристалла
	Items[2] = {5702} --3%
	-- Сундук Пурпурного Кристалла
	Items[3] = {5710} --3%
	-- Жребий Фортуны
	Items[4] = {3087} --3%
	--33% 1 приз
...
function ItemUse_ForgeItems ( role , Item )
	-- Переменная для перебора массива
	local data
	-- Проверяем предмет в первом слоте
	local CheckItem = GetChaItem ( role , 1 , 1  )
	-- Берем ID свитка
	local ScrollID = GetItemID(Item)
	-- Массив с ID свитка и ID самоцвета
	StoneByScroll = {}
	StoneByScroll = {Scroll = 5202, Gem = 5845}

	-- Перебираем массив
	for data in StoneByScroll do
		-- Если ID свитка совпадает с ID в массиве
